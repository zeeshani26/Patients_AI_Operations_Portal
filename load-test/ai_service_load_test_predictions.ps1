param(
  ...
  [string]$PatientId = "123e4567-e89b-12d3-a456-426614174001"
)
...
$endpoint = "$BaseUrl/ai/predictions/$PatientId"
...
for ($i = 0; $i -lt $Requests; $i++) {
  ...
  try {
    $null = Invoke-RestMethod -Method Get -Uri $endpoint
    $sw.Stop()
...
$outFile = Join-Path -Path $outDir -ChildPath ("ai_service_load_test_predictions_{0}.txt" -f $timestamp)

$report = @"
AI Service Load Test (predictions)
Requests: $Requests
